...
def _sentence_values(article_text: str) -> dict[int, list[dict[str, Any]]]:
...
def _local_indicator(
    ...
    if not usable:
        return None
    matched = _nearest_scope(usable, value_start, value_end)
...
def assign_roles(
    ...
    sentences = {
        row["sentence_id"]: row["text"] for row in sentence_offset_map(article_text)
    }
    values_by_sentence = _sentence_values(article_text)
