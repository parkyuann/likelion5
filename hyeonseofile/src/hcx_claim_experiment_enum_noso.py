"""HCX runner — enum labels via prompt, structured output OFF, thinking OFF.

Self-contained variant (does NOT import the original). Purpose: run HCX-007 with the
enum-augmented prompt (allowed-label list) but WITHOUT structured output and WITHOUT
thinking, so it can be compared against the enum+structured-output run under the same
label guidance.

Differences from the original hcx_claim_experiment.py:
  1. SYSTEM_PROMPT lists the allowed labels (enum guidance via prompt, soft constraint).
  2. RESPONSE_FORMAT_MODELS is empty -> no responseFormat for any model (structured output OFF).
  3. THINKING_OFF_MODELS = {"HCX-007"} -> HCX-007 sends thinking={"effort":"none"} (thinking OFF).
  4. Default --prompt-version is "claim-observation-v2-enum-noso" for a fresh cache.

CLAIM_SCHEMA carries enum too but is never sent while responseFormat is off (kept for
reference). No API call is made at import.
"""
...
# Structured output OFF for everyone; thinking OFF for HCX-007.
...
def call_hcx(
    ...
    if version == "v1":
        ...
        # Structured output OFF (RESPONSE_FORMAT_MODELS empty); thinking OFF for HCX-007.
...
def main() -> None:
    ...
    parser.add_argument("--prompt-version", default="claim-observation-v2-enum-noso")
