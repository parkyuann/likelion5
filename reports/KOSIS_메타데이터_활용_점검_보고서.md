...
- 작성일: 2026-07-30 / 개정: 2026-07-31 (구조 재편 — 전체 인벤토리를 앞으로, 기존 §1 클라이언트 절을 §1에 흡수)
- 대상: `kosis_client.py` → 크롤링(`kosis_table_tree.json`, `kosis_table_meta_v4.jsonl`) → 카탈로그(`kosis_catalog_builder.py`) → 벡터 색인(`kosis_v2_indexer.py`) 전 구간
- 근거: `KOSIS 공유서비스 개발가이드.pdf`(158p) 원문 + 소스코드·데이터 실측
- 목적: KOSIS가 제공하는 **전체 메타데이터를 분모로 놓고**, 무엇을 크롤링했고 무엇을 벡터DB에서 실제로 썼는지, 그리고 **가지고 있으나 안 쓴 데이터 / 안 크롤링한 유용 데이터**를 식별

---

## 0. 요약 (TL;DR)

1. **크롤링은 충실**하다: `getMeta`의 ITM(분류·항목)·PRD(시점)·UNIT(단위)을 표당 수집해 `kosis_table_meta_v4.jsonl`(265,082건 성공)에 저장.
...
4. **트리 단계에서 공짜 데이터를 버렸다**: 통계목록 API가 함께 주는 `SEND_DE`(최종갱신일)·`REC_TBL_SE`(추천 통계표 여부)를 저장하지 않았다.
...
## 1. KOSIS 메타데이터 전체 인벤토리 (개발가이드 PDF 기준)

이 절이 이후 모든 격차 분석의 **분모**다. 표의 두 열이 핵심이다.

- **클라이언트 지원** — `src/kosis_client.py`에 호출 함수가 있는가
- **우리 크롤링** — 실제로 데이터를 받아 저장했는가 (상세는 §2)
...
| 통계자료 메타 (2.5) | `statisticsData.do` (getMeta) | ✅ `get_meta()` — type 인자로 9종 전부 | ⚠️ 9종 중 3종만 |
| 통계설명 (2.4) | `statisticsExplData.do` | ❌ **함수 자체가 없음** | ❌ |
| 통합검색 (2.6) | `statisticsSearch.do` | ✅ `search_tables()` | ❌ **호출하되 응답 미파싱** |

> 이 외에 `get_data()`(`statisticsParameterData.do`)가 실제 통계 수치를 조회한다 — 메타가 아니라 검증 단계용.
> 2.7 지표(indicator, `pkNumberService.do` 등)는 통계표와 다른 '지표' 축이라 본 프로젝트 대상이 아니다.
...
| `TBL_NM` | 통계표명 | ✅ |
| `STAT_ID` | 통계조사ID | ✅ |
| `LIST_ID` / `LIST_NM` | 목록ID / 목록명 | ✅ (경로 `path` 조립에 사용) |
| `VW_CD` / `VW_NM` | 서비스뷰ID / 명 | ❌ (불필요) |
| **`SEND_DE`** | **최종갱신일** | ❌ **버림 — §5-B** |
| **`REC_TBL_SE`** | **추천 통계표 여부** | ❌ **버림 — §5-B** |

### 1.2 통계자료 메타 `statisticsData.do?method=getMeta` (가이드 p.138~146)

| type | 주요 출력 필드 | 우리 크롤링 |
|---|---|---|
| TBL | `TBL_NM` (+영문) | △ 표명은 트리에서 확보 |
| ORG | `ORG_NM` (기관 국문명) | △ `kosis_org_names.json`로 별도 확보 |
| PRD 수록정보 | `PRD_SE`(수록주기), `PRD_DE`(수록시점) | ✅ |
| ITM 분류항목 | `OBJ_ID, OBJ_NM, ITM_ID, ITM_NM, UP_ITM_ID, OBJ_ID_SN, UNIT_ID, UNIT_NM` | ✅ |
| UNIT 단위 | `UNIT_NM` | ✅ 조건부 호출 |
| **CMMT 주석** | **`CMMT_NM`(주석유형), `CMMT_DC`(주석 3000자)**, OBJ/ITM scope | ❌ **미사용 — §5-C** |
| **SOURCE 출처** | **`JOSA_NM`(조사명), `DEPT_NM`(담당부서), `DEPT_PHONE`, `STAT_ID`** | ❌ **미사용 — §5-C** |
| **NCD 자료갱신일** | `ORG_NM, TBL_NM, PRD_SE, PRD_DE,` **`SEND_DE`(자료갱신일)** | ❌ **미사용 — §5-C** |
| WGT 가중치 | `C1~C8, ITM_ID, WGT_CO` | ❌ (검색엔 불필요) |

### 1.3 통계설명 `statisticsExplData.do` (가이드 p.111~116) — ⭐클라이언트에 없는 API

`statId`(통계조사 ID) 단위로 **약 27개 서술형 필드**를 준다.
...
### 1.4 통합검색 `statisticsSearch.do` (가이드 p.148) — ⭐호출하면서 버리는 API
...
> **핵심**: §1.3의 서술형 메타와 §1.1의 `REC_TBL_SE`를 **표당 getMeta 여러 번 대신 검색 1회로** 상당 부분 대체할 수 있다.

---

## 2. 실제로 크롤링(추출)한 메타데이터
...
> §1.1 대비 `SEND_DE`·`REC_TBL_SE` 누락.

### 2.2 `kosis_table_meta_v4.jsonl` — `kosis_meta_enricher.py` (getMeta ITM+PRD+조건부 UNIT)

실측 265,094건 중 `ok` 265,082 / `error` 12. 성공 레코드의 필드:
...
> §1.2 대비 **CMMT·SOURCE·NCD·WGT 미호출.**

---

## 3. 벡터DB 파이프라인이 실제로 사용한 데이터

### 3.1 `kosis_catalog_builder.py` → 카탈로그 (활용도 높음)
...
|---|---|---|---|
| **`doc_item_index` (BM25/sparse)** | 카탈로그에 생성 | ❌ **sparse 색인 코드가 indexer에 없음** | "종로구" 같은 차원값 토큰 매칭 불가 |
...
| 차원값 코드 `id/up_id/sn` | meta v4에는 있음 | ❌ 카탈로그에서 `nm`만 남기고 제거 | 검색→수치조회(`get_data` objL1)로 잇는 코드 단절 |

**핵심**: 카탈로그 주석은 "dense/sparse 요구가 반대라 둘로 갈랐다"고 설계했으나, **indexer에 sparse 색인 자체가 구현돼 있지 않다.** 필터용 payload도 대부분 실려 있지 않다.

---

## 5. 크롤링하지 않은 것 중 벡터DB에 쓸 만한 데이터

### A. 이미 가진 데이터인데 색인만 안 함 (구현만 하면 즉시 활용)

`doc_item_index`(BM25/sparse), `units`, `period_types`, `latest_period`, `dimensions`, `stat_id`, `category_paths` — §4 참고.

### B. 한 번 받았다가 버린 것 (통계목록 응답에 원래 있었음 — §1.1)

- **`SEND_DE`(최종갱신일)** — 통계목록에 함께 오던 값. **getMeta 추가 호출 없이** 최신성 필터/정렬에 바로 쓸 수 있었다.
- **`REC_TBL_SE`(추천 통계표 여부)** — KOSIS가 지정한 '대표 표' 신호. 검색 랭킹 부스팅에 유용.

> 나중에 넣으려면 26만 표 재크롤링이 필요하므로 **비싼 누락**이다.

### C. 크롤링 안 했으나 가치 큰 서술형 메타 (§1.2·§1.3)
...
3. **NCD `SEND_DE`** — B와 동일 목적(최신성). 트리에서 못 받았으면 여기서 보강 가능.
...
| 순위 | 작업 | 근거 | 난이도 |
|---|---|---|---|
| 1 | **indexer에 sparse(BM25) 벡터 + 필터 payload 복원** | 카탈로그가 이미 `doc_item_index`·units·period를 만들어 둠. 재크롤링 불필요 | 낮음 |
| 2 | **트리 크롤러에 `SEND_DE`·`REC_TBL_SE` 추가** | 통계목록 응답에 원래 포함, 공짜 | 낮음 |
| 3 | **서술형 메타 확보**: 통합검색 `CONTENTS`/`ITEM03` 파싱 (우선) 또는 CMMT·통계설명 크롤링 | 검색 정확도 개선 효과 최대 | 중간 |
...
- 데이터 실측: `data/kosis_table_meta_v4.jsonl` 265,094행(ok 265,082 / error 12) 필드 집계
