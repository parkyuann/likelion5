...
def value_candidates_by_sentence(article_text: str) -> dict[int, list[str]]:
...
def chunk_sentence_ids(
    ...
    ids = [row["sentence_id"] for row in sentence_offset_map(article_text)]
    if not ids:
        return [[]]
...
def build_l2_prompt(
    ...
    values = value_candidates_by_sentence(article_text)
    lines = []
...
def build_l2_schema(
    ...
    all_ids = [row["sentence_id"] for row in sentence_offset_map(article_text)]
    ...
    *,
    ...
    for target_ids in chunk_sentence_ids(article_text, chunk_size):
        ...
        prediction, usage, chunk_latency = _call_hcx_json(
            ...
            user_prompt=build_l2_prompt(title, article_text, target_ids),
            schema=build_l2_schema(article_text, target_ids),
            api_key=api_key,
            model=model,
            timeout=timeout,
        ...
        for target_ids in chunk_sentence_ids(article_text, chunk_size):
            ...
            prediction, usage, chunk_latency = _call_hcx_json(
                ...
                schema=_split_schema(
                    article_text, target_ids, pass_name=pass_name
                ),
                api_key=api_key,
                model=model,
                timeout=timeout,
