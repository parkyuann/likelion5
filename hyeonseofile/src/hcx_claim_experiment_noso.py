"""HCX claim-extraction experiment runner — structured output DISABLED (arm A).

Self-contained variant of hcx_claim_experiment.py. It does NOT import the original
module and can fully replace it. It exists to run the "no structured output" arm of
the HCX-007 A/B experiment (structured output ON vs OFF), holding the prompt and
schema definition identical to the original so the ONLY variable is structured output.

Differences from the original hcx_claim_experiment.py:

  1. RESPONSE_FORMAT_MODELS is empty, so NO model (including HCX-007) attaches
     `responseFormat`. Every request becomes a plain prompt-only chat completion whose
     JSON is recovered from free text by parse_content() — i.e. structured output OFF.
  2. The default --prompt-version is "claim-observation-noso" so this run gets a fresh
     experiment_id / cache and does not collide with the structured-output run
     (which used "claim-observation-v1").

Everything else — SYSTEM_PROMPT, CLAIM_SCHEMA (kept for reference but never sent while
responseFormat is off), metrics, caching — is identical to the original.

rule-based candidate CSV -> HCX extraction -> Codex silver comparison. Each run is
isolated by experiment_id and records latency, token usage, estimated cost, raw
prediction, and evaluation metrics. No API call is made at import.
"""
...
# Structured output DISABLED: no model attaches responseFormat, so HCX-007 runs
# prompt-only just like the other v3 models. This is the "OFF" arm of the A/B test.
...
def call_hcx(
    ...
    if version == "v1":
        ...
        # Structured output OFF: RESPONSE_FORMAT_MODELS is empty, so responseFormat is
        # never attached (not even for HCX-007). Prompt-only path -> parse_content().
...
def main() -> None:
    ...
    parser.add_argument("--prompt-version", default="claim-observation-noso")
