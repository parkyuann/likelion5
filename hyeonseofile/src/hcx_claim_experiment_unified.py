"""Unified HCX claim-extraction experiment runner.

One file, three switches — replaces the five near-duplicate variants
(hcx_claim_experiment.py / _enum / _noso / _noso_nothink / _enum_noso).
Every model and every configuration runs through the SAME code path, so
comparisons are apples-to-apples.

Switches (all apply meaningfully only to HCX-007; other models are always
prompt-only because they do not support responseFormat / thinking):

  --structured-output {on,off}   HCX-007 sends responseFormat (JSON schema). default: on
  --thinking          {on,off}   HCX-007 reasoning. off = send thinking:effort=none. default: off
  --enum              {on,off}   allowed-label enum in prompt (all models) + schema (HCX-007). default: off

Old-file equivalents (same request body, verified):
  original            = --structured-output on  --thinking off --enum off
  _enum               = --structured-output on  --thinking off --enum on
  _noso (confounded)  = --structured-output off --thinking on  --enum off
  _noso_nothink       = --structured-output off --thinking off --enum off
  _enum_noso          = --structured-output off --thinking off --enum on

The experiment_id embeds the switch combo, so each configuration gets its own
output folder / cache and never collides. No API call is made at import.
"""
...
# Only these models accept the respective v3 features. The switches are gated to
# them so forcing a feature on an unsupported model does not send an invalid body.
RESPONSE_FORMAT_CAPABLE = {"HCX-007"}
THINKING_CAPABLE = {"HCX-007"}

# Allowed-label vocabularies. Source of truth: src/retrieval_schema.py
# (CLAIM_CLASSES / SOURCE_SCOPES / SOURCE_ROLES) plus "노이즈" (gold non-claim class).
CLAIM_CLASS_ENUM = [
...
def build_schema(use_enum: bool) -> dict:
    """CLAIM_SCHEMA with or without enum constraints (identical otherwise)."""
    return {
        "type": "object",
        "properties": {
            "is_claim": {"type": "boolean"},
            "claim_class": {"type": "string", **({"enum": CLAIM_CLASS_ENUM} if use_enum else {})},
            ...
            "observations": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "relation_type": {"type": "string", **({"enum": RELATION_TYPE_ENUM} if use_enum else {})},
            ...
            "source_scope": {"type": ["string", "null"], **({"enum": SOURCE_SCOPE_ENUM + [None]} if use_enum else {})},
            "source_role": {"type": ["string", "null"], **({"enum": SOURCE_ROLE_ENUM + [None]} if use_enum else {})},
            "evidence_quote": {"type": "string"},
        },
        "required": [
            "is_claim", "claim_class", "indicator_raw", "population", "observations",
            "source_org_raw", "source_scope", "source_role", "evidence_quote",
        ],
    }
...
def build_prompt(use_enum: bool) -> str:
    return _PROMPT_BASE + _PROMPT_ENUM_SUFFIX if use_enum else _PROMPT_BASE
...
def experiment_id(model: str, temperature: float, top_p: float, max_tokens: int,
                  prompt_version: str, config_tag: str) -> str:
    raw = f"{model}|{temperature}|{top_p}|{max_tokens}|{prompt_version}|{config_tag}"
    digest = hashlib.sha1(raw.encode("utf-8")).hexdigest()[:10]
    safe_model = model.replace("/", "-").replace(" ", "-")
    return f"{safe_model}_t{temperature:g}_p{top_p:g}_{prompt_version}_{config_tag}_{digest}"
...
def tokenize_messages(api_key: str, model: str, api_version: str, messages: list[dict],
                      timeout: int = 30) -> tuple[int, float]:
...
def call_hcx(claim_text: str, api_key: str, model: str, temperature: float, top_p: float,
             max_tokens: int, prompt: str, schema: dict, structured_output: bool,
             thinking_off: bool, api_version: str = "auto", timeout: int = 120) -> tuple[dict, dict, float]:
    ...
    if version == "v1":
        ...
        # Switches apply only to capable models (HCX-007). Others stay prompt-only.
        if structured_output and model in RESPONSE_FORMAT_CAPABLE:
            body["responseFormat"] = {"type": "json", "schema": schema}
        if thinking_off and model in THINKING_CAPABLE:
    ...
    if response.status_code >= 400:
        detail = response.text.strip().replace("\n", " ")[:1000]
        raise RuntimeError(f"HCX request failed ({response.status_code}) url={url} body={detail}")
...
def calculate_rlt_score(metrics: dict, latency_ref_ms: float, tokens_ref: float,
                        recall_weight: float, latency_weight: float, tokens_weight: float) -> float | None:
    ...
    score = recall ** recall_weight * latency_efficiency ** latency_weight * token_efficiency ** tokens_weight
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Unified HCX experiment runner (switches: structured-output / thinking / enum)")
    ...
    # --- the three switches ---
    parser.add_argument("--structured-output", choices=["on", "off"], default="on",
                        help="HCX-007 responseFormat(JSON schema) 사용 여부 (default: on)")
    parser.add_argument("--thinking", choices=["on", "off"], default="off",
                        help="HCX-007 추론 사용 여부. off=thinking effort none 전송 (default: off)")
    parser.add_argument("--enum", choices=["on", "off"], default="off",
                        help="허용 라벨 enum: 프롬프트(전 모델)+스키마(HCX-007) (default: off)")
    parser.add_argument("--prompt-version", default="claim-observation")
    parser.add_argument("--system-prompt-file", type=Path, help="지정 시 --enum 프롬프트 대신 이 파일 사용")
    ...
    use_so = args.structured_output == "on"
    thinking_off = args.thinking == "off"
    use_enum = args.enum == "on"
    schema = build_schema(use_enum)
    prompt = args.system_prompt_file.read_text(encoding="utf-8") if args.system_prompt_file else build_prompt(use_enum)
    config_tag = f"so-{args.structured_output}_th-{args.thinking}_en-{args.enum}"

    args.model = normalize_model_name(args.model)
    exp_id = experiment_id(args.model, args.temperature, args.top_p, args.max_tokens, args.prompt_version, config_tag)
                ...
                try:
                    if args.dry_run:
                        ...
                        prediction, usage, latency_ms = call_hcx(
                            ...
                            args.temperature, args.top_p, args.max_tokens, prompt, schema,
                            use_so, thinking_off, args.api_version,
                        )
