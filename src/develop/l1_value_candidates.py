...
프로젝트 사양 6.4절 names this a layer and freezes it at recall 0.968, but it had
...
def sentence_offset_map(
    article_text: str,
    *,
    sentence_span_iterator: Callable[[str], Iterator[tuple[int, int, int, str]]] | None = None,
) -> list[dict[str, Any]]:
    iterator = iter_sentence_spans if sentence_span_iterator is None else sentence_span_iterator
    return [
        {"sentence_id": index, "char_start": start, "char_end": end, "text": sentence}
        for index, start, end, sentence in iterator(article_text)
    ]
