"""이미 크롤링해 둔 표 목록에, KOSIS API로 '차원·항목·시점·단위'를 덧붙이는 보강 크롤러 (v2).

    kosis_tree_crawler.py         : statisticsList.do 로 트리를 훑어 '표 목록'을 만든다(전체 크롤링).
    kosis_tree_enricher.py        : 표마다 getMeta ITM+PRD 로 '차원·항목·시점'을 채운다.
    kosis_meta_enricher.py(이) : 여기에 '단위(UNIT)'까지 채운다.

트리(kosis_table_tree.json)에는 표당 org_id / 표이름 / stat_id / 카테고리경로 뿐이라,
검색 색인·매핑을 만들려면 아래가 더 필요하다. 모두 목록 API에 없고 getMeta에만 있다.

  ① type=ITM   차원명·차원값·항목명·단위   → doc_meta_text / doc_item_index / units
  ② type=PRD   수록주기·시작~최신시점       → period_types / latest_period
               (표이름·차원·항목까지 똑같고 조사연도만 다른 표들을 가르는 단서)
  ③ type=UNIT  단위(%, 명, 백만원 …)         → units 보강

왜 UNIT을 따로 부르나 (v1과의 차이):
  단위는 대부분 ITM 응답의 항목 행(UNIT_NM)에 함께 온다. 그러나 '비율·지수'처럼 항목이
  단일한 표는 ITM 응답에 UNIT_NM 이 비어 오고, 단위(%)가 getMeta UNIT 에만 있다
  (실측: DT_101094A001 '누적 혼인 비율' → ITM 단위 없음, UNIT=[{"UNIT_NM":"%"}]).
  단위는 매핑에서 '건수(명) vs 비율(%)'처럼 지표어가 비슷한 표를 가르는 신호라 중요하다.

  전부에 UNIT을 부르면 표당 3회가 되어 시간이 크게 는다. 그래서 v2는 '조건부'로 부른다 —
  ITM 에서 단위를 이미 얻었으면 UNIT 을 생략하고, 비었을 때만 UNIT 을 추가 호출한다
  (표본 515건 실측: 53%는 ITM 만으로 단위 확보 → UNIT 호출은 나머지 47%에만).
  --always-unit 로 무조건 호출하도록 바꿀 수 있다.

표당 API 호출: ITM(1) + PRD(1) + UNIT(단위 없을 때만 0~1) = 2~3회. 분당 200건 제한
(2026-07-15 시행, 개발가이드 1.4.2 에러코드 40)은 API 키 단위라 여러 대로 나눠도 합산된다
— 같은 키로 동시에 두 곳에서 돌리지 말 것.

출력은 트리를 덮어쓰지 않고 별도 JSONL(kosis_table_meta.jsonl)에 append 하며,
재실행하면 이미 받은 표는 건너뛴다(중단·재개 가능). 범위는 --categories / --org-id /
--limit / --whole-paths 로 좁힌다.

사용 예 (레포 루트에서):
    # 0) 응답 형태부터 확인 (원본 JSON, 몇 건만)
    venv/Scripts/python.exe src/kosis_meta_enricher.py --limit 2 --dump-raw

    # 1) 대분류 하나만 보강 (인구 = A)
    venv/Scripts/python.exe src/kosis_meta_enricher.py --categories A
...
# 차원·항목·시점·단위까지 보강한 결과라 별도 파일(_enriched)에 쓴다.
DEFAULT_OUTPUT = ROOT / "data" / "kosis_table_meta_enriched.jsonl"
...
def sample_per_category(tables: list[dict], per_category: int, seed: int) -> list[dict]:
    """top category(대분류)마다 랜덤으로 per_category개씩 뽑는다.

    표본이 한 분야(인구)에 몰리면 전 분야 검색 성능을 못 본다. 30개 대분류에서
    골고루 뽑아 커버리지를 확보한다. 카테고리 '안'에서도 앞 순서(대표 표)만 뽑히지
    않게 랜덤 추출하며, seed로 재현 가능하게 고정한다. 표가 per_category개 미만인
    카테고리는 있는 만큼만 담는다. 결과는 카테고리 순서로 이어 붙인다
    (인구 20 → 사회일반 20 → …; 순서는 색인·검색에 영향 없고 진척 파악에 편하다).
    """
    import random
    ...
    for table in tables:
        by_cat[table["top_id"]].append(table)

    rng = random.Random(seed)
    selected: list[dict] = []
    for top_id in by_cat:  # 트리(dict) 순서 = 대분류 정의 순서
        group = by_cat[top_id]
        picked = rng.sample(group, min(per_category, len(group)))
...
def parse_unit_rows(rows: list) -> list[str]:
    """getMeta(type=UNIT) 응답에서 단위 국문명 목록을 뽑는다.

    응답 예: [{"UNIT_NM":"%","UNIT_NM_ENG":"%"}]. 영문명은 버린다.
    """
    units = {str(r.get("UNIT_NM") or "").strip() for r in rows if isinstance(r, dict)}
...
def fetch_one(table: dict, limiter: RateLimiter, *, with_prd: bool, with_unit: bool,
              always_unit: bool, dump_raw: bool) -> dict:
    """표 1개의 메타(ITM + 선택적 PRD + 조건부/항상 UNIT)를 조회한다.

    실패해도 예외를 올리지 않는다. UNIT 은 기본적으로 ITM 에서 단위를 못 얻었을 때만
    부른다(조건부). always_unit=True 면 항상 부른다.
    """
    base = {
    ...
    record["unit_source"] = "itm" if record["units"] else "none"

    if with_prd:
        prd_rows, perr = call_with_retry(limiter, table["org_id"], table["tbl_id"], "PRD")
        ...
        if prd_rows is None:
            record.update({"periods": [], "period_types": [], "latest_period": None, "prd_error": perr})
        else:
            record.update(parse_prd_rows(prd_rows))

    # UNIT: ITM에서 단위를 못 얻었을 때만(조건부), 또는 always_unit이면 무조건 호출.
    if with_unit and (always_unit or not record["units"]):
        unit_rows, uerr = call_with_retry(limiter, table["org_id"], table["tbl_id"], "UNIT")
        if dump_raw and unit_rows is not None:
            print(json.dumps({"table_key": table["table_key"], "UNIT": unit_rows}, ensure_ascii=False, indent=1), flush=True)
        if unit_rows is None:
            record["unit_error"] = uerr
        else:
            unit_units = parse_unit_rows(unit_rows)
            if unit_units:
                merged = sorted(set(record["units"]) | set(unit_units))
                record["units"] = merged
                record["unit_source"] = "itm+unit" if record["unit_source"] == "itm" else "unit"

    return record


def main() -> None:
    parser = argparse.ArgumentParser(description="KOSIS 표별 차원·항목·시점·단위 보강 크롤러 v2")
    ...
    parser.add_argument("--per-category", type=int, default=None,
                        help="top category(대분류)마다 랜덤 N개씩 표본 추출 "
                             "(예: 20 → 30개 분류 × 20 = 600개). --seed로 재현")
    ...
    parser.add_argument("--skip-unit", action="store_true",
                        help="UNIT(단위) 보강을 건너뛴다 — v1과 동일하게 ITM 단위만 사용")
    parser.add_argument("--always-unit", action="store_true",
                        help="단위가 이미 있어도 UNIT을 항상 호출한다(표당 호출 1회 증가). "
                             "기본은 ITM 단위가 비었을 때만 호출(조건부)")
    ...
    if args.per_category is not None:
        # 카테고리별 랜덤 표본. 이미 뽑아 done에 들어간 것도 재현성 위해 전체에서 뽑은 뒤
        # done을 제외하는 순서로 처리한다(중단·재개해도 같은 표본을 겨냥).
        sampled = sample_per_category(tables, args.per_category, args.seed)
        pending = [t for t in sampled if t["table_key"] not in done]
    ...
    # 표당 호출 수: ITM(1) + PRD(1) + UNIT(조건부 0~1, always면 1). 조건부는 평균을
    # 알 수 없어 하한(UNIT 0회)과 상한(UNIT 1회)으로 예상 소요를 범위로 보여준다.
    limiter = RateLimiter(args.max_per_min)
    base_calls = 1 + (1 if with_prd else 0)
    unit_max = 1 if with_unit else 0
    unit_min = unit_max if args.always_unit else 0
    est_lo = len(pending) * (base_calls + unit_min) * limiter.interval
    est_hi = len(pending) * (base_calls + unit_max) * limiter.interval

    print(
        f"[STATUS] 대상 표 {len(tables):,} | 이미 완료 {len(done):,} | "
        f"이번 실행 {len(pending):,} | PRD {'포함' if with_prd else '생략'} | "
        f"UNIT {'항상' if (with_unit and args.always_unit) else '조건부' if with_unit else '생략'} | "
        f"출력 {args.output}",
        flush=True,
    )
    if est_lo == est_hi:
    ...
    ok_count = 0
    err_count = 0
    unit_calls = 0  # 조건부 UNIT이 실제로 몇 번 불렸는지(비용 추적)
    ...
    def save_checkpoint(status: str, processed: int, last_key: str | None) -> None:
        write_checkpoint(ckpt_path, {
            ...
            "completed_tables": len(done) + ok_count,
    ...
    print(
        ...
        f"소요 {format_elapsed(time.time() - started_at)} | 체크포인트 {ckpt_path.name} ===",
        flush=True,
    )
