...
데이터: `data/kosis_catalog_v1.jsonl`(전체 265,094표), `data/kosis_catalog_v2.jsonl`(메타 보강 표본 515표), `data/claims_v1.jsonl`(claim 17,686건)
...
- → 통계표 265,094개 전수 카탈로그 + 515개 표본 메타 보강으로 EDA 수행
...
- 표의 83%가 **2개 이상의 차원**(성별 × 연령별 × 행정구역별 ...)을 가짐
- 차원 하나의 값 개수는 중앙값 18개, 최대 **7,898개** — 표 하나가 수만 개 셀
- 가장 흔한 차원: **성별, 연령별, 행정구역별** — 뉴스가 흔히 언급하는 단서와 정확히 일치
...
- 수록 주기는 **년 / 5년 / 월 / 분기 / 부정기** 5종으로 수렴
- 단위도 명, 가구, %, 건 등 소수 어휘로 수렴 (단 '여자백명당남자수' 같은 파생 단위 존재)
...
- 단위 어휘 비교: KOSIS 표는 절대량(명, 가구) 중심, 뉴스는 **%·원 중심** —
  뉴스는 원자료가 아니라 **파생값(증감률)** 을 말한다는 뜻 → 단순 값 대조로는 검증 불가
- **→ 설계 반영**:
...
| ③ | 표의 83%가 다차원 격자 (성별·연령·지역) | fig04, 06 | `population_*`, `dimension_json`으로 차원 단서 구조화 |
...
- 남은 과제: 메타 보강 표본 515개 → 전체 확대, 파생 단위('여자백명당남자수' 등) 정규화 사전 확장

---

## 발표 시 예상 질문 대비

**Q. 515개 표본으로 차원 구조를 일반화해도 되나?**
A. 차원·항목 메타는 표별 API 호출(`getMeta`)이 필요해 인구 도메인 중심 표본으로 시작했다. 분류 깊이·주제·기관 분포는 전체 265,094개 기준이며, 차원 구조 통계는 "인구 관련 표 기준"이라고 한정해 말하면 안전하다.
